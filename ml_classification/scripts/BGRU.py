...
from ml_classification.model.deepLearning.BidirectionalGRUClassifier import BidirectionalGRUClassifier
...
if __name__ == "__main__":
    ...
    num_epoch = 15
    learning_rate = 1e-3
    model_path = '../model_save/BGRU.ckpt'
    ...
    # 创建MLPClassifier实例
    BGRUClassifier = BidirectionalGRUClassifier(input_dim, 41, hidden_layers, hidden_dim).to(device)
    ...
    # 训练模型
    train( BGRUClassifier,train_loader, val_loader, num_epoch, learning_rate, model_path, device, writer)
    ...
    # 构建带时间戳的文件名
    output_file = f'../result/BGRUprediction_{timestamp}.csv'
    # 使用带时间戳的文件名调用函数
    test( BGRUClassifier, test_loader, device, output_file)