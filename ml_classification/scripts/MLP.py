...
from ml_classification.model.deepLearning.MLPClassfier import MLPClassifier
...
if __name__ == "__main__":
    ...
    num_epoch = 15
    learning_rate = 1e-3
    model_path = '../model_save/MLP.ckpt'
    ...
    # 创建MLPClassifier实例
    simpleClassifier = MLPClassifier(input_dim, 41, hidden_layers, hidden_dim).to(device)
    ...
    # 构建带时间戳的文件名
    output_file = f'../result/MLPprediction_{hidden_dim}_{timestamp}.csv'
    # 使用带时间戳的文件名调用函数
    test(simpleClassifier, test_loader, device, output_file)